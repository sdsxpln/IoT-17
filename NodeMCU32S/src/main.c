...
void app_main() {
	...
	gpio_pullup_en((gpio_num_t)GPIO_INPUT_IO_TRIGGER);		// use pullup on GPIO
	gpio_pulldown_dis((gpio_num_t)GPIO_INPUT_IO_TRIGGER);       // not use pulldown on GPIO

	esp_deep_sleep_enable_ext0_wakeup((gpio_num_t)GPIO_INPUT_IO_TRIGGER, 0); // Wake if GPIO is low
